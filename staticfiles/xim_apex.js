document.addEventListener('DOMContentLoaded', () => {
    ...
    const diagSubMotivo3 = document.getElementById('diagSubMotivo3'); // Corregido: asignación correcta
    ...
    const despachoUbigeo = document.getElementById('despachoUbigeo'); // Este es un input de texto
    ...
    let rowCount = 0; // Declaración única de rowCount aquí
    ...
    // Nueva estructura de datos para el diagnóstico
    const diagnosticoData = {
		...
		"PRECIO Y CONTENIDO": {
			...
			"NO DESEA RENOVAR": [], // Sub Motivo de Cancelación 1 (final)
            "PLANES CAROS": [],
    ...
    // Lógica para Tipo de Solicitud (No necesita listener de cambio, es fijo)
...
if (diagSubMotivo2) {
    ...
    if (searchButton) {
        searchButton.addEventListener('click', () => {
            ...
            if (query.length > 0) {
                ...
                alert('Por favor, ingrese un DNI o Teléfono para buscar.');
                displayClientData({
    ...
    if (newClientForm) {
        newClientForm.addEventListener('submit', (event) => {
            ...
            if (errors.length > 0) {
                ...
                alert('Cliente creado exitosamente: ' + name + ' (DNI: ' + docNumber + ', Teléfono: ' + phone + ')');
                newClientModal.style.display = 'none';
                newClientForm.reset();
            }
        });
    ...
    // addRowBtn ya está declarado arriba, no se redeclara aquí.
    if (addRowBtn) { // Asegúrate de que el botón existe antes de añadir el listener
    ...
    if (guardarOportunidadBtn) {
        guardarOportunidadBtn.addEventListener('click', () => {
            ...
            if (confirm('¿Está seguro de guardar la oportunidad?')) {
    ...
    // Asegura que la pantalla de login esté visible al inicio y el menú principal oculto.
